...
int octtodec(int *number, bool *base){
  int i, decimal0, z = 0, index = 0, aux;
  aux = *number;
  if(*number==0)
    i=1;
  else{
...
int OctalBasic(int *number, int *Num, int *suboperation){
  int i, z, index, loc;
  setbuf(stdin, NULL);
  int Number1, Number2, Result, decimal0[2], decimal1[2], aux;
...
void BinMenu(int *number, int *operation, int *suboperation, int *Num, bool *base){
  ...
  if(*operation==1){
    switch(*suboperation){
      ...
      break;
      case(2): //substract
        cout<<"Enter the other number you want to subb (this number in binary*): ";
        cin>>*Num;
        setbuf(stdin, NULL);
        Result = BinBasic(number, Num, suboperation);
        *base = !true;
